...
OrderIdentification AS (
    SELECT 
        oi.ORDER_ID,
        oi.ID_VALUE AS SHOPIFY_ORDER_NAME
    FROM order_identification oi
    WHERE oi.ORDER_IDENTIFICATION_TYPE_ID = 'SHOPIFY_ORD_NAME'
),
...
FilteredProducts AS (
    SELECT 
        product_id
    FROM product
    WHERE product_type_id IN ('DIGITAL_GOOD', 'DONATION')
)
SELECT DISTINCT 
    sum(ra.DISCOUNT),
...
JOIN OrderIdentification oi ON rd.HC_ORDER_ID = oi.ORDER_ID
...
WHERE rd.PRODUCT_ID NOT IN (SELECT PRODUCT_ID FROM FilteredProducts)
and rd.return_date BETWEEN '2024-05-01' AND '2024-05-31'
